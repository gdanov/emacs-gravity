...
4. **Pull-based terminal protocol** — Server only ever sends lightweight `state-changed` signals; clients fetch data on demand via `poll`. Inbox/notice signals are polled immediately (user-blocking actions surface at once); session/overview are debounced to prevent UI freezes during heavy editing. Push terminal communication has been removed — there is no longer a `GRAVITY_PUSH_MODE`.
