...
describe("InboxManager", () => {
  ...
  describe("forceClosePlanReviewForSession", () => {
    it("force-closes plan-review with pending socket", () => {
      const sock = mockSocket();
      const item = inbox.add("plan-review", "s1", "proj", "Plan", "s", {}, sock);
      const removed = inbox.forceClosePlanReviewForSession("s1");
      expect(removed).toHaveLength(1);
      expect(removed[0].id).toBe(item.id);
      expect(sock.written).toHaveLength(1);
      expect(JSON.parse(sock.written[0].replace("\n", ""))).toEqual({});
    ...
    it("removes plan-review without pending socket", () => {
      inbox.add("plan-review", "s1", "proj", "Plan", "s", {});
      const removed = inbox.forceClosePlanReviewForSession("s1");
      expect(removed).toHaveLength(1);
      expect(inbox.all()).toHaveLength(0);
    });

    it("does not affect permission items with pending socket", () => {
      const sock = mockSocket();
      inbox.add("permission", "s1", "proj", "Read", "s", {}, sock);
      const removed = inbox.forceClosePlanReviewForSession("s1");
      expect(removed).toHaveLength(0);
    ...
    it("removes only plan-review when mixed with permission", () => {
      const permSock = mockSocket();
      const planSock = mockSocket();
      inbox.add("permission", "s1", "proj", "Read", "s", {}, permSock);
      inbox.add("plan-review", "s1", "proj", "Plan", "s", {}, planSock);
      const removed = inbox.forceClosePlanReviewForSession("s1");
      expect(removed).toHaveLength(1);
      expect(removed[0].type).toBe("plan-review");
      expect(planSock.ended).toBe(true);
      expect(permSock.ended).toBe(false);
      expect(inbox.all()).toHaveLength(1);
      expect(inbox.all()[0].type).toBe("permission");
    });

    it("does not affect other session's plan-review", () => {
      const sock1 = mockSocket();
      const sock2 = mockSocket();
      inbox.add("plan-review", "s1", "proj", "Plan1", "s", {}, sock1);
      inbox.add("plan-review", "s2", "proj", "Plan2", "s", {}, sock2);
      const removed = inbox.forceClosePlanReviewForSession("s1");
      expect(removed).toHaveLength(1);
      expect(removed[0].label).toBe("Plan1");
      expect(sock1.ended).toBe(true);
      expect(sock2.ended).toBe(false);
      expect(inbox.all()).toHaveLength(1);
    });
    ...
    it("handles already-closed socket gracefully", () => {
      ...
      sock.write = () => { throw new Error("socket closed"); };
      sock.end = () => { throw new Error("socket closed"); };
      inbox.add("plan-review", "s1", "proj", "Plan", "s", {}, sock);
      const removed = inbox.forceClosePlanReviewForSession("s1");
      expect(removed).toHaveLength(1);
      expect(inbox.all()).toHaveLength(0);
    });
    ...
    it("returns empty array for unknown session", () => {
      const removed = inbox.forceClosePlanReviewForSession("unknown");
      expect(removed).toHaveLength(0);
    });
  });
