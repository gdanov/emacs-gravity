...
| `error` | log; no patch | |
| `compaction_end` | emit `Compaction` → `addCompaction` primitive → `add_compaction` patch; marker pushed to `session.compactions[]`. | Captures reason/turnNumber/tokensBefore/summary/aborted. Does NOT move turn boundaries — marker attaches to whichever turn is current when compaction completes. |
| `queue_update`, `compaction_start`, `auto_retry_start` / `auto_retry_end`, `extension_error` | (log to stderr; no patches) | Documented in `docs/rpc.md`. `compaction_start` is informational — the marker is recorded only on `_end`. |
| `tool_execution_update` | emit `ToolPartial` → `update_tool_partial` patch; latest partial lands on `tool.partial`. Also stashed on accumulator so `accToolEnd` can fall back to it if pi's `_end` carries no `result`. | Cumulative-snapshot model: each `_update` replaces (does not append). Terminals choose how to render `tool.partial`; CC tools always have `partial: null`. |
