...
(ert-deftest pi-test-pre-tool-use-captures-tool-with-turn ()
  ...
  (let ((sid "pi-tool-test-1"))
    ...
    (let* ((session (claude-gravity--get-session sid))
      ...
      (should turn)
      (let ((steps (alist-get 'steps turn)))
        (should steps)
        (let ((step (car (claude-gravity--tlist-items steps))))
          (should step)
          (let ((tools (alist-get 'tools step)))
            (should tools)
...
(ert-deftest pi-test-post-tool-use-completes-tool ()
  ...
  (let ((sid "pi-tool-test-2"))
    ...
    (let* ((session (claude-gravity--get-session sid))
           (turns (plist-get session :turns))
           (turn (car (claude-gravity--tlist-items turns)))
           (steps (alist-get 'steps turn))
           (step (car (claude-gravity--tlist-items steps)))
           (tools (alist-get 'tools step))
  ...
  (let ((sid "pi-tool-attr-1"))
    ...
    (let* ((session (claude-gravity--get-session sid))
      ...
      (let* ((turn1 (car turn-list))
             (steps1 (alist-get 'steps turn1))
             (step1 (car (claude-gravity--tlist-items steps1)))
             (tools1 (alist-get 'tools step1))
      ...
      (let* ((turn2 (cadr turn-list))
             (steps2 (alist-get 'steps turn2))
             (step2 (car (claude-gravity--tlist-items steps2)))
             (tools2 (alist-get 'tools step2))
